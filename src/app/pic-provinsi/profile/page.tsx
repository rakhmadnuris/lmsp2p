  ...
  return (
    <div style={{maxWidth:'600px',margin:'0 auto'}}>
      ...
      <div className="glass-panel" style={{marginBottom:'1.5rem',textAlign:'center'}}>
        ...
        <div style={{color:'var(--secondary)',fontSize:'0.8rem',marginBottom:'1rem'}}>{user.role==='PIC_PROVINSI'?'PIC Provinsi':'PIC Kabupaten/Kota'} — {user.province}{user.regencyCity?` / ${user.regencyCity}`:''}</div>
      ...
      <div className="glass-panel">
        ...
        <form onSubmit={handleSave} style={{display:'flex',flexDirection:'column',gap:'0.75rem'}}>
          ...
          <div>
            ...
            <input type="password" className="input-field" value={newPassword} onChange={e=>setNewPassword(e.target.value)} placeholder="Password baru (kosongkan jika tidak diganti)" />
          </div>
          <div>
          ...
          <button type="submit" disabled={saving} className="btn btn-primary" style={{marginTop:'0.5rem'}}>{saving?'Menyimpan...':'Simpan Perubahan'}</button>
        </form>
      </div>
    </div>
  );
