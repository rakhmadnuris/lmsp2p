...
export default function Stage3() {
  ...
  useEffect(() => {
    fetch('/api/participant/progress')
      .then(res => res.json())
      .then(data => {
        if (data.progress?.stage3Score !== null && data.progress?.stage3Score !== undefined) {
          setScore(data.progress.stage3Score);
        }
      ...
      .finally(() => {
        // Load draft answers
        const draft = localStorage.getItem('stage3Answers');
        if (draft) {
          try { setAnswers(JSON.parse(draft)); } catch (e) {}
        }
        
        // Load timer state
        const savedStartTime = localStorage.getItem('stage3StartTime');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('stage3Answers', JSON.stringify(answers));
  }, [answers, initialLoad, isLocked]);

  const startTest = () => {
    const now = Date.now();
    localStorage.setItem('stage3StartTime', now.toString());
    setHasStarted(true);
  ...
  const autoSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/participant/progress', {
        ...
        body: JSON.stringify({ stage: 3, data: { score: calculatedScore } })
      });
      if (res.ok) {
        setScore(calculatedScore);
        localStorage.removeItem('stage3Answers');
        localStorage.removeItem('stage3StartTime');
        setTimeout(() => router.push('/stages/4'), 3000);
      }
    } catch (err) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div className="glass-panel" style={{ textAlign: 'center' }}>
          ...
          <div style={{ marginTop: '2rem' }}>
            <button onClick={() => router.push('/stages/4')} className="btn btn-primary">Lanjut ke Tahap 4</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div className="glass-panel" style={{ textAlign: 'center' }}>
          <h1 style={{ color: 'var(--secondary)', marginBottom: '1rem', fontSize: '3rem' }}>{score}/100</h1>
          <h2>Pre-Test Selesai!</h2>
          <p style={{ color: 'var(--text-muted)', marginTop: '1rem', marginBottom: '2rem' }}>Mengalihkan ke Tahap 4...</p>
          <button onClick={() => router.push('/stages/4')} className="btn btn-primary">Ke Tahap 4 Sekarang</button>
        </div>
      </div>
    );
  }
      ...
      {!hasStarted && !isLocked ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
          <div className="glass-panel" style={{ textAlign: 'center', maxWidth: '500px' }}>
            <h1 style={{ marginBottom: '1rem' }}>Tahap 3: Pre-Test</h1>
